...
import { MODELS_CACHE_FILE, STATE_DIR } from "../constants";
...
function promptText(prompt: unknown): string {
  ...
  for (const block of blocks) {
    ...
    if (type === "text" && typeof obj.text === "string") {
      ...
      const label =
        typeof obj.title === "string"
          ...
          : typeof obj.name === "string"
            ? obj.name
            : uri;
      parts.push(`<resource_link uri="${escapeAttr(uri)}" title="${escapeAttr(label)}"/>`);
    } else if (
      type === "resource" &&
      obj.resource &&
      ...
      if (text) parts.push(`<embedded_resource uri="${escapeAttr(uri)}">\n${text}\n</embedded_resource>`);
